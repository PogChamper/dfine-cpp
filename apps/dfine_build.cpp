// dfine_build — build a TensorRT .engine from a D-FINE ONNX model entirely in C++.
//
// The canonical engines are produced by the Python pipeline (build_engine.py); this
// is the equivalent pure-C++ path. It copies the descriptive <onnx>.json sidecar
// through to <engine>.json, updating precision/batch fields.
//
// Only FP32 is offered: it is the validated D-FINE precision. FP16 needs the
// decoder pinned to FP32 (a naive global kFP16 flag corrupts it) and INT8 needs
// QDQ-based explicit quantization — both are out of scope and, if pursued, belong
// in the Python pipeline where they can be mAP-validated.
//
// usage:
...
struct Args {
    ...
    int opt_batch{1};
    int max_batch{1};
    bool cuda_graph{false};  // advisory sidecar label only (capture is a runtime concern)
    bool verbose{false};
};
...
void usage(const char* argv0) {
    std::fprintf(stderr,
                 ...
                 "  --cuda-graph         Label the sidecar cuda_graph_compat=true (advisory)\n"
