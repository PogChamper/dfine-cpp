...
- **Interrupted writes cannot poison consumers.** The exported ONNX, both FP16-converted
  graphs, the engine plan, and every sidecar are each staged in a temp file and moved into
  place atomically; a failed export gate deletes its staging instead of leaving a consumable
  artifact next to a stale sidecar. (The engine and its sidecar are two files swapped in
  sequence — each swap is atomic, the pair is not jointly transactional.)
