...
- **Interrupted writes are far harder to weaponize.** Every artifact write in the toolchain
  (the exported ONNX, all converted graphs, the engine plan, and every sidecar — exporter,
  fp16/fp16-legacy/int8 converters, builder) stages in a temp file and lands via an atomic
  rename, and a failed export gate deletes its staging instead of leaving a consumable
  artifact next to a stale sidecar. Caveat: each graph+sidecar PAIR is still two adjacent
  renames, not one transaction — a crash exactly between them can pair a new file with the
  previous metadata; both files are fully staged first, so the window is two syscalls.
