...
class LQE(nn.Module):
    ...
    def _topk_no_sort(self, x, k):
        top_values = []
        for _ in range(k):
            index = x.argmax(dim=-1, keepdim=True)
            value = x.gather(-1, index)
            top_values.append(value)
            x = x.scatter(-1, index, torch.zeros_like(value))
        return torch.cat(top_values, dim=-1)
...
class DFINETransformer(nn.Module):
    ...
    def __init__(
        ...
        # Retained for strict compatibility with training checkpoints.
    ...
    def _get_decoder_input(
        ...
        # Multiplication preserves exporter dtype inference.
    ...
    def forward(self, feats):
        if self.training:
            raise RuntimeError("the bundled D-FINE model supports inference only")
        memory, spatial_shapes = self._get_encoder_input(feats)
        init_ref_contents, init_ref_points_unact, _, _ = self._get_decoder_input(
            memory, spatial_shapes
        )
        out_bboxes, out_logits, _, _, _, _, _ = self.decoder(
            init_ref_contents,
